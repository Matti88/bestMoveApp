...
export async function fetchGeoapifyData(address:string) {
    ...
    const apiUrl = `https://api.geoapify.com/v1/geocode/search?text=${encodeURIComponent(address)}&format=json&apiKey=${process.env.NEXT_PUBLIC_GEO_APIFY_KEY!}`;
...
export async function fetchGeoapifyIsochrones(lat:number, lon:number, mode:string, time:number) {
  const apiUrl =`https://api.geoapify.com/v1/isoline?lat=${lat}&lon=${lon}&type=time&mode=${mode}&range=${time}&apiKey=${process.env.NEXT_PUBLIC_GEO_APIFY_KEY!}`
